...
__global__ void energy(int* d_aid, int* d_csrList, int* d_offset, int* d_layer, int* d_energy, int* d_max_vertex, int layer)
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
...
layer_0_layering<<< ceil(float(V)/1024), 1024>>> (d_apr, d_layer, V);
layer_0_activate<<< ceil(float(V)/1024), 1024>>> (d_aid, d_layer);
layer_1_energy  <<< ceil(float(V)/1024), 1024>>> (d_csrList, d_offset, d_layer, d_energy, d_max_vertex);
layer_1_boundary<<< 1, 1 >>>                     (d_layer, d_max_vertex);

for(int ii=1; ii<(L-1); ii++)
{
    vertex_activate    <<< ceil(float(V)/1024), 1024>>> (d_apr, d_layer, d_aid, d_energy, ii);
    energy             <<< ceil(float(V)/1024), 1024>>> (d_aid, d_csrList, d_offset, d_layer, d_energy, d_max_vertex, ii);
    vertex_deactivate  <<< ceil(float(V)/1024), 1024>>> (d_csrList, d_offset, d_layer, d_aid, d_energy, ii);
    next_layer_boundary<<< 1, 1 >>>                     (d_layer, d_max_vertex, ii);
}

vertex_activate    <<< ceil(float(V)/1024), 1024>>> (d_apr, d_layer, d_aid, d_energy, L-1);
last_layer_deactivate <<< ceil(float(V)/1024), 1024>>> (d_layer, d_aid, L);
find_number_active <<< ceil(float(V)/1024), 1024>>> (d_aid, d_activeVertex, d_layer, L);
