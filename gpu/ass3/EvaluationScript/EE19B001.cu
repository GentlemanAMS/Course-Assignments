...
//Layer 0 activate
__global__ void layer_0_activate(int* d_aid, int* d_layer)
{
...
__global__ void layer_1_energy(int* d_csrList, int* d_offset, int* d_layer, int* d_energy, int* d_max_vertex)
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid < (d_layer[0]) + 1)
    {
...
__global__ void find_layer_boundary(int* d_csrList, int* d_offset, int* d_layer, int* d_max_vertex, int layer)
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid < (d_layer[layer]) - (d_layer[layer-1]+1) + 1)
    {
        ...
        atomicMax(&d_layer[layer+1], d_max_vertex[vertex]);
    }
}


//Initially d_aid must be set to 0 for everything except Layer 0
//Activate threads in the layer 'layer'
__global__ void energy_activate(int* d_apr, int* d_csrList, int* d_offset, int* d_layer, int* d_aid, int* d_energy, int layer)
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid < (d_layer[layer]) - (d_layer[layer-1]+1) + 1)
    {
...
cudaMemset(&d_layer, 0, L*sizeof(int));
...
layer_0_layering<<< ceil(float(V)/NO_OF_THREADS), NO_OF_THREADS>>> (d_apr, d_layer, V);
layer_0_activate<<< ceil(float(V)/NO_OF_THREADS), NO_OF_THREADS>>> (d_aid, d_layer);
layer_1_energy  <<< ceil(float(V)/NO_OF_THREADS), NO_OF_THREADS>>> (d_csrList, d_offset, d_layer, d_energy, d_max_vertex);
layer_1_boundary<<< 1, 1 >>>                     (d_layer, d_max_vertex);

for(int ii=1; ii<(L-1); ii++)
{
    find_layer_boundary    <<< ceil(float(V)/NO_OF_THREADS), NO_OF_THREADS>>> (d_csrList, d_offset, d_layer, d_max_vertex, ii);
}
...
cudaMemcpy(h_layer, d_layer, L*sizeof(int), cudaMemcpyDeviceToHost);
...
for(int ii=1; ii<(L-1); ii++)
{
    h_left_limit = h_layer[ii-1];
    h_right_limit = h_layer[ii];
    no_of_threadblocks = ceil(((float)(h_right_limit - h_left_limit + 1))/NO_OF_THREADS);
    energy_activate    <<< no_of_threadblocks, NO_OF_THREADS>>> (d_apr, d_csrList, d_offset, d_layer, d_aid, d_energy, ii);
    vertex_deactivate  <<< no_of_threadblocks, NO_OF_THREADS>>> (d_csrList, d_offset, d_layer, d_aid, d_energy, ii);
}


h_left_limit = h_layer[L-2];
h_right_limit = h_layer[L-1];
no_of_threadblocks = ceil(((float)(h_right_limit - h_left_limit + 1))/NO_OF_THREADS);
vertex_activate    <<< no_of_threadblocks, 1024>>> (d_apr, d_layer, d_aid, d_energy, L-1);
last_layer_deactivate <<< no_of_threadblocks, 1024>>> (d_layer, d_aid, L);

find_number_active <<< ceil(float(L)/1024), 1024>>> (d_aid, d_activeVertex, d_layer, L);
