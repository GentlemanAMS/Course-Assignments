...
__global__ void per_row_AB_kernel(long int *A, long int *B, long int *C,long int m, long int n){
    ...
    long int threadnumber = blockIdx.x * blockDim.x + threadIdx.x;

    long int row_index_a = threadnumber / m;
    long int row_index_b = threadnumber % m;

    long int c_row, c_column;

    if (threadnumber < m * m) {

        for (long int ii = 0; ii < n; ii++){
            for (long int jj = 0; jj < n; jj++){
                // C[row_index_a * n + ii][jj * m + row_index_b] = A[row_index_a][jj] * B[row_index_b][ii]                
                c_row = row_index_a * n + ii;
                c_column = jj * m + row_index_b;
                C[c_row * (m*n) + c_column] = A[row_index_a * n + jj] * B[row_index_b * n + ii];
            }
        }
    }
...
__global__ void per_column_AB_kernel(long int *A, long int *B, long int *C,long int m, long int n){
    ...
    //long int threadnumber = blockIdx.x * gridDim.x + threadIdx.y * blockDim.x + threadIdx.x;
    long int threadnumber = blockIdx.x * blockDim.x * blockDim.y + threadIdx.x * blockDim.y + threadIdx.y;

    long int column_index_a = threadnumber / n;
    long int column_index_b = threadnumber % n;

    if (threadnumber < n * n) {

        for (long int ii = 0; ii < m; ii++){
            for (long int jj = 0; jj < m; jj++){
                // C[ii * n + column_index_b][column_index_a * m + jj] = A[ii][column_index_a] * B[jj][column_index_b]
                long int c_row = ii * n + column_index_b;
                long int c_column = column_index_a * m + jj;
                C[c_row * (m*n) + c_column] = A[ii * n + column_index_a] * B[jj * n + column_index_b];
            }
        }
    }
}
...
__global__ void per_element_kernel(long int *A, long int *B, long int *C,long int m, long int n){
    ...
    long int threadnumber = (gridDim.y * blockIdx.x + blockIdx.y) * blockDim.x * blockDim.y + blockDim.y * threadIdx.x + threadIdx.y;
    long int c_row, c_column, a_row, a_column, b_row, b_column;
    c_row = threadnumber / (m*n);
    c_column = threadnumber % (m*n);
    a_row = c_row / n;
    a_column = c_column /m;
    b_row = c_column % m;
    b_column = c_row % n;

    // C[c_row][c_column] = A[a_row][a_column] * B[b_row][b_column];
    if (threadnumber < (m*n)*(m*n)){
        C[c_row*m*n + c_column] = A[a_row*n + a_column] * B[b_row*n + b_column];
    }
}
...
int main(int argc,char **argv){
    ...
    gridDimx = ceil(float(m * m) / 1024.0);
    dim3 grid_kernel1(gridDimx, 1, 1);
    dim3 block_kernel1(1024,1,1);    

    double starttime = rtclock();  

    // --> Launch the kernel 
    per_row_AB_kernel<<<grid_kernel1, block_kernel1>>>(d_a, d_b, d_c, m, n);
    ...
    gridDimx = ceil(float(n * n) / 1024.0);
    dim3 grid_kernel2(gridDimx, 1, 1);
    dim3 block_kernel2(32,32,1);    

    starttime = rtclock(); 

    // --> Launch the kernel

    per_column_AB_kernel<<<grid_kernel2, block_kernel2>>>(d_a, d_b, d_c, m, n);
    cudaDeviceSynchronize(); 
